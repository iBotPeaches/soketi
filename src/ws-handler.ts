...
export class WsHandler {
  ...
  async closeAllLocalSockets(): Promise<void> {
    ...
    return async
      .each(
        [...namespaces],
        ([namespaceId, namespace]: [string, Namespace], nsCallback) => {
          namespace.getSockets().then((sockets) => {
            async
              .each(
                [...sockets],
                ([wsId, ws]: [string, WebSocket<any>], wsCallback) => {
                  try {
                    ws.sendJson({
                      event: "pusher:error",
                      data: {
      ...
      this.server.adapter
        ...
        .then((members) => {
          ...
          if (!members.has(user_id as string)) {
            this.server.webhookSender.sendMemberAdded(
              ws.app,
              channel,
              user_id as string,
            )
   ...
   */
  protected signinTokenForUserData(
    ws: WebSocket<any>,
    userData: string,
  ): Promise<string> {
