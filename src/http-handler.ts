...
export class HttpHandler {
  ...
  channels(res: HttpResponse) {
    this.attachMiddleware(res, [
      ...
      this.server.adapter
        .getChannelsWithSocketsCount(res.params.appId)
        .then((channels) => {
          let response: { [channel: string]: ChannelResponse } = [
            ...
            if (
              res.query.filter_by_prefix &&
              !channel.startsWith(res.query.filter_by_prefix)
            ) {
              return channels
            }
  ...
  channel(res: HttpResponse) {
    this.attachMiddleware(res, [
      ...
      this.server.adapter
        .getChannelSocketsCount(res.params.appId, res.params.channel)
        .then((socketsCount) => {
          response = {
          ...
          if (res.params.channel.startsWith("presence-")) {
            ...
            if (response.subscription_count > 0) {
              this.server.adapter
                .getChannelMembersCount(res.params.appId, res.params.channel)
                .then((membersCount) => {
  ...
  channelUsers(res: HttpResponse) {
    this.attachMiddleware(res, [
      ...
      this.server.adapter
        .getChannelMembers(res.params.appId, res.params.channel)
        .then((members) => {
          let broadcastMessage = {
  ...
  events(res: HttpResponse) {
    this.attachMiddleware(res, [
      ...
      this.checkMessageToBroadcast(res.body as PusherApiMessage, res.app as App)
        .then((message) => {
          this.broadcastMessage(message, res.app.id)
          this.server.metricsManager.markApiMessage(res.app.id, res.body, {
            ok: true,
          })
    ...
    this.attachMiddleware(res, [
      ...
      if (batch.length > res.app.maxEventBatchSize) {
        return this.badResponse(
          res,
          `Cannot batch-send more than ${res.app.maxEventBatchSize} messages at once`,
        )
      }

      Promise.all(
        batch.map((message) =>
          this.checkMessageToBroadcast(message, res.app as App),
        ),
      )
        .then((messages) => {
  ...
  protected checkMessageToBroadcast(
    message: PusherApiMessage,
    app: App,
  ): Promise<PusherApiMessage> {
  ...
  protected appMiddleware(res: HttpResponse, next: CallableFunction): any {
    return this.server.appManager
      .findById(res.params.appId)
      .then((validApp) => {
        if (!validApp) {
          return this.notFoundResponse(
            res,
            `The app ${res.params.appId} could not be found.`,
          )
        }
  ...
  protected readRateLimitingMiddleware(
    res: HttpResponse,
    next: CallableFunction,
  ): any {
    this.server.rateLimiter
      .consumeReadRequestsPoints(1, res.app)
      .then((response) => {
  ...
  protected broadcastEventRateLimitingMiddleware(
    res: HttpResponse,
    next: CallableFunction,
  ): any {
    ...
    this.server.rateLimiter
      .consumeBackendEventPoints(Math.max(channels.length, 1), res.app)
      .then((response) => {
        if (response.canContinue) {
          for (let header in response.headers) {
            res.writeHeader(header, "" + response.headers[header])
  ...
  protected broadcastBatchEventsRateLimitingMiddleware(
    res: HttpResponse,
    next: CallableFunction,
  ): any {
    let rateLimiterPoints = res.body.batch.reduce(
    ...
    this.server.rateLimiter
      .consumeBackendEventPoints(rateLimiterPoints, res.app)
      .then((response) => {
        if (response.canContinue) {
          for (let header in response.headers) {
            res.writeHeader(header, "" + response.headers[header])
  ...
  protected sendJson(
    res: HttpResponse,
    data: any,
    status: RecognizedString = "200 OK",
  ) {
    try {
      return res
        .writeStatus(status)
