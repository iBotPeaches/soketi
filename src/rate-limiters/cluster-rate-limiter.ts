    ...
    if (cluster.isPrimary || typeof cluster.isPrimary === "undefined") {
      if (server.pm2) {
        ...
        server.discover.join(
          ...
          (rateLimiters: { [key: string]: RateLimiterAbstract }) => {
            this.rateLimiters = Object.fromEntries(
              Object.entries(rateLimiters).map(
                ([key, rateLimiterObject]: [string, any]) => {
                  return [
                    key,
        ...
        server.discover.join(
          ...
          ({ app, eventKey, points, maxPoints }: ConsumptionMessage) => {
